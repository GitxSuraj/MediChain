...
export default function AddVisitRecord({ appointment, hospitalName, onSaved }) {
  ...
  const submit = async event => { event.preventDefault(); setSaving(true); setError(""); try { await addVisitRecord(appointment.patient_id, { ...form, appointment_id: appointment.id, hospital_name: hospitalName }); onSaved?.(); setForm({ ...form, diagnosis: "", prescription: "", notes: "" }); } catch (e) { setError(e.message); } finally { setSaving(false); } };
